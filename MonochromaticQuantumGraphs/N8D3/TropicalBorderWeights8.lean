...
/-- Every declared support coordinate remains nonzero at a positive
parameter, including after the vertex-zero normalization. -/
theorem tropicalNormalizedWeights8_ne_zero_of_supported
    {t : ℝ} (ht : 0 < t) (e : EdgeN 8 3)
    (h : tropicalEdgeSupported8 e = true) :
    tropicalNormalizedWeights8 t e ≠ 0 := by
  have hraw : tropicalRawWeights8 t e ≠ 0 := by
    rw [tropicalRawWeights8, if_pos h]
    exact_mod_cast (zpow_ne_zero _ ht.ne')
  unfold tropicalNormalizedWeights8
  split_ifs
  · apply div_ne_zero
    · exact hraw
    · exact_mod_cast (tropicalRawMonoAmplitude8_pos ht e.i).ne'
  · exact hraw

/-- Exact torus support: precisely the declared 144 coordinates are nonzero. -/
def TropicalExactSupport8 (W : WeightsN 8 3 ℂ) : Prop :=
  ∀ e, W e ≠ 0 ↔ tropicalEdgeSupported8 e = true

/-- Positive members of the tropical family have exact—not merely contained—
support. -/
theorem tropicalNormalizedWeights8_exactSupport
    {t : ℝ} (ht : 0 < t) :
    TropicalExactSupport8 (tropicalNormalizedWeights8 t) := by
  intro e
  constructor
  · intro hne
    by_contra hs
    have hs0 : tropicalEdgeSupported8 e = false :=
      Bool.eq_false_of_not_eq_true hs
    exact hne (tropicalNormalizedWeights8_eq_zero_of_unsupported t e hs0)
  · exact tropicalNormalizedWeights8_ne_zero_of_supported ht e
