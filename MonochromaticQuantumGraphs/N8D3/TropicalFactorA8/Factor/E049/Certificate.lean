...
def tropicalComponentAFactorCertificate8_049 :
    LaurentFactorCertificate (κ := Fin 4) tropicalComponentACharacter8
      ...
      (tropicalComponentARawFactor8 25)
      (tropicalComponentARawFactor8 36) where
  unit := (1 : ℤ)
  unit_ne_zero := by norm_num
  shift := TropicalFactorA8.Internal.Factor049.shift
  reduction := {
    use := TropicalFactorA8.Internal.Factor049.use
    source_eq := TropicalFactorA8.Internal.Factor049.source_eq
    target_eq := TropicalFactorA8.Internal.Factor049.target_eq
  }
