import MonochromaticQuantumGraphs.N8D3.TropicalFactorA8.Factor.E061.SourceEq
import MonochromaticQuantumGraphs.N8D3.TropicalFactorA8.Factor.E061.TargetEq

/-!
# Component-A factor edge 61: certificate
-/

namespace MonochromaticQuantumGraphs.N8D3

open MonochromaticQuantumGraph
open MonochromaticQuantumGraphs
open MonochromaticQuantumGraphs.FactorCoverCertificate
open scoped BigOperators

noncomputable section

set_option maxRecDepth 100000
set_option maxHeartbeats 8000000

/-- Exact Laurent factor certificate for raw edge 61. -/
def tropicalComponentAFactorCertificate8_061 :
    LaurentFactorCertificate (κ := Fin 4) tropicalComponentACharacter8
      (tropicalComponentAQuotientRelation8 61)
      (tropicalComponentARawFactor8 39)
      (tropicalComponentARawFactor8 46) where
  unit := (1 : ℤ)
  unit_ne_zero := by norm_num
  shift := TropicalFactorA8.Internal.Factor061.shift
  reduction := {
    use := TropicalFactorA8.Internal.Factor061.use
    source_eq := TropicalFactorA8.Internal.Factor061.source_eq
    target_eq := TropicalFactorA8.Internal.Factor061.target_eq
  }

end

end MonochromaticQuantumGraphs.N8D3
