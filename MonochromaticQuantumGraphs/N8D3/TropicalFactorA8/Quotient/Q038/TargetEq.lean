import MonochromaticQuantumGraphs.N8D3.TropicalFactorA8.Quotient.Q038.Uses

/-!
# Component-A quotient row 38: normalized target equality
-/

namespace MonochromaticQuantumGraphs.N8D3

open MonochromaticQuantumGraph
open MonochromaticQuantumGraphs
open MonochromaticQuantumGraphs.FactorCoverCertificate
open scoped BigOperators

noncomputable section

set_option maxRecDepth 100000
set_option maxHeartbeats 8000000

namespace TropicalFactorA8.Internal.Quotient038

theorem target_eq :
    (∑ k : Fin 12,
      Finsupp.single (use k).targetExponent
        (signedCoefficient (use k).reduction.signExponent
          (use k).coefficient)) =
      (1 : ℤ) • tropicalComponentAQuotientRelation8 38 := by
  simp [use, monomial00, monomial01, monomial02, monomial03, monomial04, monomial05, monomial06, monomial07, monomial08, monomial09, monomial10, monomial11, tropicalComponentAQuotientRelation8,
    signedCoefficient, Fin.sum_univ_succ] <;> abel

end TropicalFactorA8.Internal.Quotient038

end

end MonochromaticQuantumGraphs.N8D3
