...
def shiftedCertificate :
    LaurentPolynomial.ShiftedCombinationCertificate (κ := Fin 2)
      ...
      (tropicalComponentAQuotientIntermediate8 39) where
  use := shiftedUse
...
/-- Exact two-source shifted combination for quotient row 39. -/
...
def tropicalComponentAQuotientReductionCertificate8_039 :
    NormalizedCharacterReductionCertificate (κ := Fin 12)
      ...
      (tropicalComponentAQuotientIntermediate8 39)
      (tropicalComponentAQuotientRelation8 39) where
  unit := (1 : ℤ)
  unit_ne_zero := by norm_num
  reduction := {
    use := TropicalFactorA8.Internal.Quotient039.use
    source_eq := TropicalFactorA8.Internal.Quotient039.source_eq
    target_eq := TropicalFactorA8.Internal.Quotient039.target_eq
  }
