...
def shiftedCertificate :
    LaurentPolynomial.ShiftedCombinationCertificate (κ := Fin 2)
      ...
      (tropicalComponentAQuotientIntermediate8 23) where
  use := shiftedUse
...
/-- Exact two-source shifted combination for quotient row 23. -/
...
def tropicalComponentAQuotientReductionCertificate8_023 :
    NormalizedCharacterReductionCertificate (κ := Fin 12)
      ...
      (tropicalComponentAQuotientIntermediate8 23)
      (tropicalComponentAQuotientRelation8 23) where
  unit := (1 : ℤ)
  unit_ne_zero := by norm_num
  reduction := {
    use := TropicalFactorA8.Internal.Quotient023.use
    source_eq := TropicalFactorA8.Internal.Quotient023.source_eq
    target_eq := TropicalFactorA8.Internal.Quotient023.target_eq
  }
