...
def shiftedCertificate :
    LaurentPolynomial.ShiftedCombinationCertificate (κ := Fin 2)
      ...
      (tropicalComponentAQuotientIntermediate8 63) where
  use := shiftedUse
...
/-- Exact two-source shifted combination for quotient row 63. -/
...
def tropicalComponentAQuotientReductionCertificate8_063 :
    NormalizedCharacterReductionCertificate (κ := Fin 12)
      ...
      (tropicalComponentAQuotientIntermediate8 63)
      (tropicalComponentAQuotientRelation8 63) where
  unit := (1 : ℤ)
  unit_ne_zero := by norm_num
  reduction := {
    use := TropicalFactorA8.Internal.Quotient063.use
    source_eq := TropicalFactorA8.Internal.Quotient063.source_eq
    target_eq := TropicalFactorA8.Internal.Quotient063.target_eq
  }
