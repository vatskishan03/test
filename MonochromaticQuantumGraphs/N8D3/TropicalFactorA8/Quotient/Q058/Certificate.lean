...
def shiftedCertificate :
    LaurentPolynomial.ShiftedCombinationCertificate (κ := Fin 2)
      ...
      (tropicalComponentAQuotientIntermediate8 58) where
  use := shiftedUse
...
/-- Exact two-source shifted combination for quotient row 58. -/
...
def tropicalComponentAQuotientReductionCertificate8_058 :
    NormalizedCharacterReductionCertificate (κ := Fin 12)
      ...
      (tropicalComponentAQuotientIntermediate8 58)
      (tropicalComponentAQuotientRelation8 58) where
  unit := (1 : ℤ)
  unit_ne_zero := by norm_num
  reduction := {
    use := TropicalFactorA8.Internal.Quotient058.use
    source_eq := TropicalFactorA8.Internal.Quotient058.source_eq
    target_eq := TropicalFactorA8.Internal.Quotient058.target_eq
  }
