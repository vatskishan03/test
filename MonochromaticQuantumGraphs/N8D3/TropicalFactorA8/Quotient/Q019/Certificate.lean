...
def shiftedCertificate :
    LaurentPolynomial.ShiftedCombinationCertificate (κ := Fin 2)
      ...
      (tropicalComponentAQuotientIntermediate8 19) where
  use := shiftedUse
...
/-- Exact two-source shifted combination for quotient row 19. -/
...
def tropicalComponentAQuotientReductionCertificate8_019 :
    NormalizedCharacterReductionCertificate (κ := Fin 12)
      ...
      (tropicalComponentAQuotientIntermediate8 19)
      (tropicalComponentAQuotientRelation8 19) where
  unit := (1 : ℤ)
  unit_ne_zero := by norm_num
  reduction := {
    use := TropicalFactorA8.Internal.Quotient019.use
    source_eq := TropicalFactorA8.Internal.Quotient019.source_eq
    target_eq := TropicalFactorA8.Internal.Quotient019.target_eq
  }
