...
/-- A tensor-coordinate polynomial vanishing on every perfect-matching tensor
with the canonical support also vanishes at `GHZ₃`. -/
theorem polynomial_eq_zero_on_tropicalSupportTensorImage8_imp_eval_ghz_eq_zero
    (p : TropicalTensorPolynomial8)
    (hvanish : ∀ T ∈ tropicalSupportTensorImage8,
  ...
  have hsubset : tropicalSupportTensorImage8 ⊆ zeroSet := by
    intro T hT
    exact hvanish T hT
  exact (closure_minimal hsubset hzeroSet)
    ghzTensor8_mem_closure_tropicalSupportTensorImage8
...
theorem polynomial_eq_zero_on_all_perfectMatchingTensor8_imp_eval_ghz_eq_zero
    ...
    MvPolynomial.eval ghzTensor8 p = 0 := by
  apply polynomial_eq_zero_on_tropicalSupportTensorImage8_imp_eval_ghz_eq_zero p
  intro T hT
