...
/-- Tensors realized by weights supported on the canonical six-matching 144 entries. -/
...
/-- Tensors realized by weights whose nonzero coordinates are exactly the
canonical six-matching 144-entry support. -/
def tropicalExactSupportTensorImage8 : Set TropicalTensor8 :=
  {T | ∃ W : WeightsN 8 3 ℂ,
    TropicalExactSupport8 W ∧ T = perfectMatchingTensor8 W}

theorem tropicalNormalizedTensor8_mem_exactSupportImage
    {t : ℝ} (ht : 0 < t) :
    tropicalNormalizedTensor8 t ∈ tropicalExactSupportTensorImage8 := by
  exact ⟨tropicalNormalizedWeights8 t,
    tropicalNormalizedWeights8_exactSupport ht, rfl⟩

/-- **Tropical border theorem.** The three-color GHZ tensor on eight vertices
lies in the closure of the perfect-matching tensor image on the canonical six-matching
...
/-- Strong support-specific border theorem: `GHZ₃` is approached by tensors
whose weights are nonzero on every one of the 144 declared coordinates and
zero everywhere else. -/
theorem ghzTensor8_mem_closure_tropicalExactSupportTensorImage8 :
    ghzTensor8 ∈ closure tropicalExactSupportTensorImage8 := by
  apply mem_closure_of_tendsto tropicalNormalizedTensor8_tendsto_ghz
  filter_upwards [eventually_gt_atTop (0 : ℝ)] with t ht
  exact tropicalNormalizedTensor8_mem_exactSupportImage ht
