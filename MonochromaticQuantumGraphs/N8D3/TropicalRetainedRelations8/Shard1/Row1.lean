import MonochromaticQuantumGraphs.N8D3.TropicalRetainedRelations8.Data

/-! Kernel replay for first-overlap row 6. -/

namespace MonochromaticQuantumGraphs.N8D3

noncomputable section

set_option maxRecDepth 100000

/-- Exact two-base-source provenance for overlap row 6. -/
def tropicalOverlapProvenance8Row6 : TropicalOverlapProvenance8 :=
  { epsilon := 1, sourceI := 1, coordinateB := 141, sourceJ := 61, coordinateA := 138 }

/-- The explicit sparse target polynomial in overlap row 6. -/
def tropicalOverlapRelation8Row6 : LaurentPolynomial (Fin 144) :=
  Finsupp.single
      (tropicalOverlapDegreeFiveExponent8 7 64 108 126 141) 1 +
    (-Finsupp.single
      (tropicalOverlapDegreeFiveExponent8 7 64 109 126 138) 1) +
    Finsupp.single
      (tropicalOverlapDegreeFiveExponent8 17 51 85 126 141) 1 +
    (-Finsupp.single
      (tropicalOverlapDegreeFiveExponent8 17 52 85 126 138) 1) +
    Finsupp.single
      (tropicalOverlapDegreeFiveExponent8 25 51 75 116 141) 1 +
    (-Finsupp.single
      (tropicalOverlapDegreeFiveExponent8 25 52 75 116 138) 1)

/-- Explicit shifted exponents of the six monomials from source `B_i`. -/
def tropicalOverlapSourceIExponent8Row6 :
    Fin 6 → LaurentExponent (Fin 144) :=
![
  tropicalOverlapDegreeFiveExponent8 7 64 98 138 141,
  tropicalOverlapDegreeFiveExponent8 7 64 108 126 141,
  tropicalOverlapDegreeFiveExponent8 7 75 85 138 141,
  tropicalOverlapDegreeFiveExponent8 17 41 85 138 141,
  tropicalOverlapDegreeFiveExponent8 17 51 85 126 141,
  tropicalOverlapDegreeFiveExponent8 25 51 75 116 141
]

/-- Explicit shifted exponents of the six monomials from source `B_j`. -/
def tropicalOverlapSourceJExponent8Row6 :
    Fin 6 → LaurentExponent (Fin 144) :=
![
  tropicalOverlapDegreeFiveExponent8 7 64 98 138 141,
  tropicalOverlapDegreeFiveExponent8 7 64 109 126 138,
  tropicalOverlapDegreeFiveExponent8 7 75 85 138 141,
  tropicalOverlapDegreeFiveExponent8 17 41 85 138 141,
  tropicalOverlapDegreeFiveExponent8 17 52 85 126 138,
  tropicalOverlapDegreeFiveExponent8 25 52 75 116 138
]

set_option maxHeartbeats 10000000 in
/-- Kernel replay of the six shifted `B_i` exponents in row 6. -/
theorem tropicalOverlapSourceIExponent8_replay_row6 (j : Fin 6) :
    Pi.single tropicalOverlapProvenance8Row6.coordinateB (1 : ℤ) +
        tropicalMatchingLocalExponent8
          (tropicalBaseColoring8 tropicalOverlapProvenance8Row6.sourceI)
          (tropicalBaseMatching8 j) =
      tropicalOverlapSourceIExponent8Row6 j := by
  revert j
  decide

set_option maxHeartbeats 10000000 in
/-- Kernel replay of the six shifted `B_j` exponents in row 6. -/
theorem tropicalOverlapSourceJExponent8_replay_row6 (j : Fin 6) :
    Pi.single tropicalOverlapProvenance8Row6.coordinateA (1 : ℤ) +
        tropicalMatchingLocalExponent8
          (tropicalBaseColoring8 tropicalOverlapProvenance8Row6.sourceJ)
          (tropicalBaseMatching8 j) =
      tropicalOverlapSourceJExponent8Row6 j := by
  revert j
  decide

/-- The exact translated-source combination after exponent replay. -/
def tropicalOverlapSourceCombination8Row6 :
    LaurentPolynomial (Fin 144) :=
  tropicalOverlapProvenance8Row6.epsilon •
    ((∑ j : Fin 6,
        Finsupp.single (tropicalOverlapSourceIExponent8Row6 j) 1) -
      ∑ j : Fin 6,
        Finsupp.single (tropicalOverlapSourceJExponent8Row6 j) 1)

/-- Coefficientwise cancellation of the three common translated faces. -/
private theorem tropicalOverlapRelation8_sourceCombination_row6 :
    tropicalOverlapRelation8Row6 =
      tropicalOverlapSourceCombination8Row6 := by
  simp [tropicalOverlapRelation8Row6,
    tropicalOverlapSourceCombination8Row6,
    tropicalOverlapProvenance8Row6,
    tropicalOverlapSourceIExponent8Row6,
    tropicalOverlapSourceJExponent8Row6, Fin.sum_univ_succ]
  abel

/-- Staged kernel replay of `T_r = epsilon_r * (x_b B_i - x_a B_j)` for
row 6. -/
theorem tropicalOverlapRelation8_provenance_row6 :
    tropicalOverlapRelation8Row6 =
      tropicalOverlapProvenancePolynomial8
        tropicalOverlapProvenance8Row6 := by
  rw [tropicalOverlapRelation8_sourceCombination_row6]
  unfold tropicalOverlapSourceCombination8Row6
  unfold tropicalOverlapProvenancePolynomial8
  rw [tropicalOverlapTranslateBaseRelation8,
    tropicalOverlapTranslateBaseRelation8]
  simp_rw [tropicalOverlapSourceIExponent8_replay_row6,
    tropicalOverlapSourceJExponent8_replay_row6]

end

end MonochromaticQuantumGraphs.N8D3
