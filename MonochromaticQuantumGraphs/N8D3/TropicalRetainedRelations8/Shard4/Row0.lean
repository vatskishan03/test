import MonochromaticQuantumGraphs.N8D3.TropicalRetainedRelations8.Data

/-! Kernel replay for first-overlap row 20. -/

namespace MonochromaticQuantumGraphs.N8D3

noncomputable section

set_option maxRecDepth 100000

/-- Exact two-base-source provenance for overlap row 20. -/
def tropicalOverlapProvenance8Row20 : TropicalOverlapProvenance8 :=
  { epsilon := 1, sourceI := 4, coordinateB := 89, sourceJ := 8, coordinateA := 86 }

/-- The explicit sparse target polynomial in overlap row 20. -/
def tropicalOverlapRelation8Row20 : LaurentPolynomial (Fin 144) :=
  Finsupp.single
      (tropicalOverlapDegreeFiveExponent8 1 58 89 101 138) 1 +
    Finsupp.single
      (tropicalOverlapDegreeFiveExponent8 1 58 89 111 126) 1 +
    (-Finsupp.single
      (tropicalOverlapDegreeFiveExponent8 1 59 86 101 138) 1) +
    (-Finsupp.single
      (tropicalOverlapDegreeFiveExponent8 1 59 86 111 126) 1) +
    Finsupp.single
      (tropicalOverlapDegreeFiveExponent8 19 51 69 89 119) 1 +
    (-Finsupp.single
      (tropicalOverlapDegreeFiveExponent8 20 51 69 86 119) 1)

/-- Explicit shifted exponents of the six monomials from source `B_i`. -/
def tropicalOverlapSourceIExponent8Row20 :
    Fin 6 → LaurentExponent (Fin 144) :=
![
  tropicalOverlapDegreeFiveExponent8 1 58 89 101 138,
  tropicalOverlapDegreeFiveExponent8 1 58 89 111 126,
  tropicalOverlapDegreeFiveExponent8 1 69 86 89 138,
  tropicalOverlapDegreeFiveExponent8 9 41 86 89 138,
  tropicalOverlapDegreeFiveExponent8 9 51 86 89 126,
  tropicalOverlapDegreeFiveExponent8 19 51 69 89 119
]

/-- Explicit shifted exponents of the six monomials from source `B_j`. -/
def tropicalOverlapSourceJExponent8Row20 :
    Fin 6 → LaurentExponent (Fin 144) :=
![
  tropicalOverlapDegreeFiveExponent8 1 59 86 101 138,
  tropicalOverlapDegreeFiveExponent8 1 59 86 111 126,
  tropicalOverlapDegreeFiveExponent8 1 69 86 89 138,
  tropicalOverlapDegreeFiveExponent8 9 41 86 89 138,
  tropicalOverlapDegreeFiveExponent8 9 51 86 89 126,
  tropicalOverlapDegreeFiveExponent8 20 51 69 86 119
]

set_option maxHeartbeats 10000000 in
/-- Kernel replay of the six shifted `B_i` exponents in row 20. -/
theorem tropicalOverlapSourceIExponent8_replay_row20 (j : Fin 6) :
    Pi.single tropicalOverlapProvenance8Row20.coordinateB (1 : ℤ) +
        tropicalMatchingLocalExponent8
          (tropicalBaseColoring8 tropicalOverlapProvenance8Row20.sourceI)
          (tropicalBaseMatching8 j) =
      tropicalOverlapSourceIExponent8Row20 j := by
  fin_cases j <;> decide

set_option maxHeartbeats 10000000 in
/-- Kernel replay of the six shifted `B_j` exponents in row 20. -/
theorem tropicalOverlapSourceJExponent8_replay_row20 (j : Fin 6) :
    Pi.single tropicalOverlapProvenance8Row20.coordinateA (1 : ℤ) +
        tropicalMatchingLocalExponent8
          (tropicalBaseColoring8 tropicalOverlapProvenance8Row20.sourceJ)
          (tropicalBaseMatching8 j) =
      tropicalOverlapSourceJExponent8Row20 j := by
  fin_cases j <;> decide

/-- The exact translated-source combination after exponent replay. -/
def tropicalOverlapSourceCombination8Row20 :
    LaurentPolynomial (Fin 144) :=
  tropicalOverlapProvenance8Row20.epsilon •
    ((∑ j : Fin 6,
        Finsupp.single (tropicalOverlapSourceIExponent8Row20 j) 1) -
      ∑ j : Fin 6,
        Finsupp.single (tropicalOverlapSourceJExponent8Row20 j) 1)

/-- Coefficientwise cancellation of the three common translated faces. -/
private theorem tropicalOverlapRelation8_sourceCombination_row20 :
    tropicalOverlapRelation8Row20 =
      tropicalOverlapSourceCombination8Row20 := by
  simp [tropicalOverlapRelation8Row20,
    tropicalOverlapSourceCombination8Row20,
    tropicalOverlapProvenance8Row20,
    tropicalOverlapSourceIExponent8Row20,
    tropicalOverlapSourceJExponent8Row20, Fin.sum_univ_succ]
  abel

/-- Staged kernel replay of `T_r = epsilon_r * (x_b B_i - x_a B_j)` for
row 20. -/
theorem tropicalOverlapRelation8_provenance_row20 :
    tropicalOverlapRelation8Row20 =
      tropicalOverlapProvenancePolynomial8
        tropicalOverlapProvenance8Row20 := by
  rw [tropicalOverlapRelation8_sourceCombination_row20]
  unfold tropicalOverlapSourceCombination8Row20
  unfold tropicalOverlapProvenancePolynomial8
  rw [tropicalOverlapTranslateBaseRelation8,
    tropicalOverlapTranslateBaseRelation8]
  simp_rw [tropicalOverlapSourceIExponent8_replay_row20,
    tropicalOverlapSourceJExponent8_replay_row20]

end

end MonochromaticQuantumGraphs.N8D3
