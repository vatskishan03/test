import MonochromaticQuantumGraphs.N8D3.TropicalRetainedRelations8.Data

/-! Kernel replay for first-overlap row 155. -/

namespace MonochromaticQuantumGraphs.N8D3

noncomputable section

set_option maxRecDepth 100000

/-- Exact two-base-source provenance for overlap row 155. -/
def tropicalOverlapProvenance8Row155 : TropicalOverlapProvenance8 :=
  { epsilon := -1, sourceI := 54, coordinateB := 7, sourceJ := 55, coordinateA := 1 }

/-- The explicit sparse target polynomial in overlap row 155. -/
def tropicalOverlapRelation8Row155 : LaurentPolynomial (Fin 144) :=
  Finsupp.single
      (tropicalOverlapDegreeFiveExponent8 1 17 43 87 138) 1 +
    Finsupp.single
      (tropicalOverlapDegreeFiveExponent8 1 17 51 87 132) 1 +
    Finsupp.single
      (tropicalOverlapDegreeFiveExponent8 1 25 51 77 122) 1 +
    (-Finsupp.single
      (tropicalOverlapDegreeFiveExponent8 7 11 43 87 138) 1) +
    (-Finsupp.single
      (tropicalOverlapDegreeFiveExponent8 7 11 51 87 132) 1) +
    (-Finsupp.single
      (tropicalOverlapDegreeFiveExponent8 7 19 51 77 122) 1)

/-- Explicit shifted exponents of the six monomials from source `B_i`. -/
def tropicalOverlapSourceIExponent8Row155 :
    Fin 6 → LaurentExponent (Fin 144) :=
![
  tropicalOverlapDegreeFiveExponent8 1 7 64 106 138,
  tropicalOverlapDegreeFiveExponent8 1 7 64 114 132,
  tropicalOverlapDegreeFiveExponent8 1 7 77 87 138,
  tropicalOverlapDegreeFiveExponent8 7 11 43 87 138,
  tropicalOverlapDegreeFiveExponent8 7 11 51 87 132,
  tropicalOverlapDegreeFiveExponent8 7 19 51 77 122
]

/-- Explicit shifted exponents of the six monomials from source `B_j`. -/
def tropicalOverlapSourceJExponent8Row155 :
    Fin 6 → LaurentExponent (Fin 144) :=
![
  tropicalOverlapDegreeFiveExponent8 1 7 64 106 138,
  tropicalOverlapDegreeFiveExponent8 1 7 64 114 132,
  tropicalOverlapDegreeFiveExponent8 1 7 77 87 138,
  tropicalOverlapDegreeFiveExponent8 1 17 43 87 138,
  tropicalOverlapDegreeFiveExponent8 1 17 51 87 132,
  tropicalOverlapDegreeFiveExponent8 1 25 51 77 122
]

set_option maxHeartbeats 10000000 in
/-- Kernel replay of the six shifted `B_i` exponents in row 155. -/
theorem tropicalOverlapSourceIExponent8_replay_row155 (j : Fin 6) :
    Pi.single tropicalOverlapProvenance8Row155.coordinateB (1 : ℤ) +
        tropicalMatchingLocalExponent8
          (tropicalBaseColoring8 tropicalOverlapProvenance8Row155.sourceI)
          (tropicalBaseMatching8 j) =
      tropicalOverlapSourceIExponent8Row155 j := by
  revert j
  decide

set_option maxHeartbeats 10000000 in
/-- Kernel replay of the six shifted `B_j` exponents in row 155. -/
theorem tropicalOverlapSourceJExponent8_replay_row155 (j : Fin 6) :
    Pi.single tropicalOverlapProvenance8Row155.coordinateA (1 : ℤ) +
        tropicalMatchingLocalExponent8
          (tropicalBaseColoring8 tropicalOverlapProvenance8Row155.sourceJ)
          (tropicalBaseMatching8 j) =
      tropicalOverlapSourceJExponent8Row155 j := by
  revert j
  decide

/-- The exact translated-source combination after exponent replay. -/
def tropicalOverlapSourceCombination8Row155 :
    LaurentPolynomial (Fin 144) :=
  tropicalOverlapProvenance8Row155.epsilon •
    ((∑ j : Fin 6,
        Finsupp.single (tropicalOverlapSourceIExponent8Row155 j) 1) -
      ∑ j : Fin 6,
        Finsupp.single (tropicalOverlapSourceJExponent8Row155 j) 1)

/-- Coefficientwise cancellation of the three common translated faces. -/
private theorem tropicalOverlapRelation8_sourceCombination_row155 :
    tropicalOverlapRelation8Row155 =
      tropicalOverlapSourceCombination8Row155 := by
  simp [tropicalOverlapRelation8Row155,
    tropicalOverlapSourceCombination8Row155,
    tropicalOverlapProvenance8Row155,
    tropicalOverlapSourceIExponent8Row155,
    tropicalOverlapSourceJExponent8Row155, Fin.sum_univ_succ]
  abel

/-- Staged kernel replay of `T_r = epsilon_r * (x_b B_i - x_a B_j)` for
row 155. -/
theorem tropicalOverlapRelation8_provenance_row155 :
    tropicalOverlapRelation8Row155 =
      tropicalOverlapProvenancePolynomial8
        tropicalOverlapProvenance8Row155 := by
  rw [tropicalOverlapRelation8_sourceCombination_row155]
  unfold tropicalOverlapSourceCombination8Row155
  unfold tropicalOverlapProvenancePolynomial8
  rw [tropicalOverlapTranslateBaseRelation8,
    tropicalOverlapTranslateBaseRelation8]
  simp_rw [tropicalOverlapSourceIExponent8_replay_row155,
    tropicalOverlapSourceJExponent8_replay_row155]

end

end MonochromaticQuantumGraphs.N8D3
