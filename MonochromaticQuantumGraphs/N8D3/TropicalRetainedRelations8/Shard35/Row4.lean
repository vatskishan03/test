import MonochromaticQuantumGraphs.N8D3.TropicalRetainedRelations8.Data

/-! Kernel replay for first-overlap row 179. -/

namespace MonochromaticQuantumGraphs.N8D3

noncomputable section

set_option maxRecDepth 100000

/-- Exact two-base-source provenance for overlap row 179. -/
def tropicalOverlapProvenance8Row179 : TropicalOverlapProvenance8 :=
  { epsilon := 1, sourceI := 66, coordinateB := 90, sourceJ := 78, coordinateA := 86 }

/-- The explicit sparse target polynomial in overlap row 179. -/
def tropicalOverlapRelation8Row179 : LaurentPolynomial (Fin 144) :=
  Finsupp.single
      (tropicalOverlapDegreeFiveExponent8 1 64 90 101 141) 1 +
    Finsupp.single
      (tropicalOverlapDegreeFiveExponent8 1 64 90 112 126) 1 +
    (-Finsupp.single
      (tropicalOverlapDegreeFiveExponent8 1 65 86 104 141) 1) +
    (-Finsupp.single
      (tropicalOverlapDegreeFiveExponent8 1 65 86 115 126) 1) +
    Finsupp.single
      (tropicalOverlapDegreeFiveExponent8 19 52 75 90 119) 1 +
    (-Finsupp.single
      (tropicalOverlapDegreeFiveExponent8 20 52 75 86 122) 1)

/-- Explicit shifted exponents of the six monomials from source `B_i`. -/
def tropicalOverlapSourceIExponent8Row179 :
    Fin 6 → LaurentExponent (Fin 144) :=
![
  tropicalOverlapDegreeFiveExponent8 1 64 90 101 141,
  tropicalOverlapDegreeFiveExponent8 1 64 90 112 126,
  tropicalOverlapDegreeFiveExponent8 1 75 86 90 141,
  tropicalOverlapDegreeFiveExponent8 11 41 86 90 141,
  tropicalOverlapDegreeFiveExponent8 11 52 86 90 126,
  tropicalOverlapDegreeFiveExponent8 19 52 75 90 119
]

/-- Explicit shifted exponents of the six monomials from source `B_j`. -/
def tropicalOverlapSourceJExponent8Row179 :
    Fin 6 → LaurentExponent (Fin 144) :=
![
  tropicalOverlapDegreeFiveExponent8 1 65 86 104 141,
  tropicalOverlapDegreeFiveExponent8 1 65 86 115 126,
  tropicalOverlapDegreeFiveExponent8 1 75 86 90 141,
  tropicalOverlapDegreeFiveExponent8 11 41 86 90 141,
  tropicalOverlapDegreeFiveExponent8 11 52 86 90 126,
  tropicalOverlapDegreeFiveExponent8 20 52 75 86 122
]

set_option maxHeartbeats 10000000 in
/-- Kernel replay of the six shifted `B_i` exponents in row 179. -/
theorem tropicalOverlapSourceIExponent8_replay_row179 (j : Fin 6) :
    Pi.single tropicalOverlapProvenance8Row179.coordinateB (1 : ℤ) +
        tropicalMatchingLocalExponent8
          (tropicalBaseColoring8 tropicalOverlapProvenance8Row179.sourceI)
          (tropicalBaseMatching8 j) =
      tropicalOverlapSourceIExponent8Row179 j := by
  fin_cases j <;> decide

set_option maxHeartbeats 10000000 in
/-- Kernel replay of the six shifted `B_j` exponents in row 179. -/
theorem tropicalOverlapSourceJExponent8_replay_row179 (j : Fin 6) :
    Pi.single tropicalOverlapProvenance8Row179.coordinateA (1 : ℤ) +
        tropicalMatchingLocalExponent8
          (tropicalBaseColoring8 tropicalOverlapProvenance8Row179.sourceJ)
          (tropicalBaseMatching8 j) =
      tropicalOverlapSourceJExponent8Row179 j := by
  fin_cases j <;> decide

/-- The exact translated-source combination after exponent replay. -/
def tropicalOverlapSourceCombination8Row179 :
    LaurentPolynomial (Fin 144) :=
  tropicalOverlapProvenance8Row179.epsilon •
    ((∑ j : Fin 6,
        Finsupp.single (tropicalOverlapSourceIExponent8Row179 j) 1) -
      ∑ j : Fin 6,
        Finsupp.single (tropicalOverlapSourceJExponent8Row179 j) 1)

/-- Coefficientwise cancellation of the three common translated faces. -/
private theorem tropicalOverlapRelation8_sourceCombination_row179 :
    tropicalOverlapRelation8Row179 =
      tropicalOverlapSourceCombination8Row179 := by
  simp [tropicalOverlapRelation8Row179,
    tropicalOverlapSourceCombination8Row179,
    tropicalOverlapProvenance8Row179,
    tropicalOverlapSourceIExponent8Row179,
    tropicalOverlapSourceJExponent8Row179, Fin.sum_univ_succ]
  abel

/-- Staged kernel replay of `T_r = epsilon_r * (x_b B_i - x_a B_j)` for
row 179. -/
theorem tropicalOverlapRelation8_provenance_row179 :
    tropicalOverlapRelation8Row179 =
      tropicalOverlapProvenancePolynomial8
        tropicalOverlapProvenance8Row179 := by
  rw [tropicalOverlapRelation8_sourceCombination_row179]
  unfold tropicalOverlapSourceCombination8Row179
  unfold tropicalOverlapProvenancePolynomial8
  rw [tropicalOverlapTranslateBaseRelation8,
    tropicalOverlapTranslateBaseRelation8]
  simp_rw [tropicalOverlapSourceIExponent8_replay_row179,
    tropicalOverlapSourceJExponent8_replay_row179]

end

end MonochromaticQuantumGraphs.N8D3
