import MonochromaticQuantumGraphs.N8D3.TropicalRetainedRelations8.Data

/-! Kernel replay for first-overlap row 112. -/

namespace MonochromaticQuantumGraphs.N8D3

noncomputable section

set_option maxRecDepth 100000

/-- Exact two-base-source provenance for overlap row 112. -/
def tropicalOverlapProvenance8Row112 : TropicalOverlapProvenance8 :=
  { epsilon := 1, sourceI := 34, coordinateB := 141, sourceJ := 94, coordinateA := 138 }

/-- The explicit sparse target polynomial in overlap row 112. -/
def tropicalOverlapRelation8Row112 : LaurentPolynomial (Fin 144) :=
  Finsupp.single
      (tropicalOverlapDegreeFiveExponent8 1 64 114 129 141) 1 +
    (-Finsupp.single
      (tropicalOverlapDegreeFiveExponent8 1 64 115 129 138) 1) +
    Finsupp.single
      (tropicalOverlapDegreeFiveExponent8 11 51 87 129 141) 1 +
    (-Finsupp.single
      (tropicalOverlapDegreeFiveExponent8 11 52 87 129 138) 1) +
    Finsupp.single
      (tropicalOverlapDegreeFiveExponent8 19 51 76 122 141) 1 +
    (-Finsupp.single
      (tropicalOverlapDegreeFiveExponent8 19 52 76 122 138) 1)

/-- Explicit shifted exponents of the six monomials from source `B_i`. -/
def tropicalOverlapSourceIExponent8Row112 :
    Fin 6 → LaurentExponent (Fin 144) :=
![
  tropicalOverlapDegreeFiveExponent8 1 64 105 138 141,
  tropicalOverlapDegreeFiveExponent8 1 64 114 129 141,
  tropicalOverlapDegreeFiveExponent8 1 76 87 138 141,
  tropicalOverlapDegreeFiveExponent8 11 42 87 138 141,
  tropicalOverlapDegreeFiveExponent8 11 51 87 129 141,
  tropicalOverlapDegreeFiveExponent8 19 51 76 122 141
]

/-- Explicit shifted exponents of the six monomials from source `B_j`. -/
def tropicalOverlapSourceJExponent8Row112 :
    Fin 6 → LaurentExponent (Fin 144) :=
![
  tropicalOverlapDegreeFiveExponent8 1 64 105 138 141,
  tropicalOverlapDegreeFiveExponent8 1 64 115 129 138,
  tropicalOverlapDegreeFiveExponent8 1 76 87 138 141,
  tropicalOverlapDegreeFiveExponent8 11 42 87 138 141,
  tropicalOverlapDegreeFiveExponent8 11 52 87 129 138,
  tropicalOverlapDegreeFiveExponent8 19 52 76 122 138
]

set_option maxHeartbeats 10000000 in
/-- Kernel replay of the six shifted `B_i` exponents in row 112. -/
theorem tropicalOverlapSourceIExponent8_replay_row112 (j : Fin 6) :
    Pi.single tropicalOverlapProvenance8Row112.coordinateB (1 : ℤ) +
        tropicalMatchingLocalExponent8
          (tropicalBaseColoring8 tropicalOverlapProvenance8Row112.sourceI)
          (tropicalBaseMatching8 j) =
      tropicalOverlapSourceIExponent8Row112 j := by
  fin_cases j <;> decide

set_option maxHeartbeats 10000000 in
/-- Kernel replay of the six shifted `B_j` exponents in row 112. -/
theorem tropicalOverlapSourceJExponent8_replay_row112 (j : Fin 6) :
    Pi.single tropicalOverlapProvenance8Row112.coordinateA (1 : ℤ) +
        tropicalMatchingLocalExponent8
          (tropicalBaseColoring8 tropicalOverlapProvenance8Row112.sourceJ)
          (tropicalBaseMatching8 j) =
      tropicalOverlapSourceJExponent8Row112 j := by
  fin_cases j <;> decide

/-- The exact translated-source combination after exponent replay. -/
def tropicalOverlapSourceCombination8Row112 :
    LaurentPolynomial (Fin 144) :=
  tropicalOverlapProvenance8Row112.epsilon •
    ((∑ j : Fin 6,
        Finsupp.single (tropicalOverlapSourceIExponent8Row112 j) 1) -
      ∑ j : Fin 6,
        Finsupp.single (tropicalOverlapSourceJExponent8Row112 j) 1)

/-- Coefficientwise cancellation of the three common translated faces. -/
private theorem tropicalOverlapRelation8_sourceCombination_row112 :
    tropicalOverlapRelation8Row112 =
      tropicalOverlapSourceCombination8Row112 := by
  simp [tropicalOverlapRelation8Row112,
    tropicalOverlapSourceCombination8Row112,
    tropicalOverlapProvenance8Row112,
    tropicalOverlapSourceIExponent8Row112,
    tropicalOverlapSourceJExponent8Row112, Fin.sum_univ_succ]
  abel

/-- Staged kernel replay of `T_r = epsilon_r * (x_b B_i - x_a B_j)` for
row 112. -/
theorem tropicalOverlapRelation8_provenance_row112 :
    tropicalOverlapRelation8Row112 =
      tropicalOverlapProvenancePolynomial8
        tropicalOverlapProvenance8Row112 := by
  rw [tropicalOverlapRelation8_sourceCombination_row112]
  unfold tropicalOverlapSourceCombination8Row112
  unfold tropicalOverlapProvenancePolynomial8
  rw [tropicalOverlapTranslateBaseRelation8,
    tropicalOverlapTranslateBaseRelation8]
  simp_rw [tropicalOverlapSourceIExponent8_replay_row112,
    tropicalOverlapSourceJExponent8_replay_row112]

end

end MonochromaticQuantumGraphs.N8D3
