import MonochromaticQuantumGraphs.N8D3.TropicalRetainedRelations8.Data

/-! Kernel replay for first-overlap row 73. -/

namespace MonochromaticQuantumGraphs.N8D3

noncomputable section

set_option maxRecDepth 100000

/-- Exact two-base-source provenance for overlap row 73. -/
def tropicalOverlapProvenance8Row73 : TropicalOverlapProvenance8 :=
  { epsilon := 1, sourceI := 21, coordinateB := 89, sourceJ := 31, coordinateA := 85 }

/-- The explicit sparse target polynomial in overlap row 73. -/
def tropicalOverlapRelation8Row73 : LaurentPolynomial (Fin 144) :=
  Finsupp.single
      (tropicalOverlapDegreeFiveExponent8 7 64 89 99 138) 1 +
    Finsupp.single
      (tropicalOverlapDegreeFiveExponent8 7 64 89 108 129) 1 +
    (-Finsupp.single
      (tropicalOverlapDegreeFiveExponent8 7 65 85 102 138) 1) +
    (-Finsupp.single
      (tropicalOverlapDegreeFiveExponent8 7 65 85 111 129) 1) +
    Finsupp.single
      (tropicalOverlapDegreeFiveExponent8 25 51 76 89 116) 1 +
    (-Finsupp.single
      (tropicalOverlapDegreeFiveExponent8 26 51 76 85 119) 1)

/-- Explicit shifted exponents of the six monomials from source `B_i`. -/
def tropicalOverlapSourceIExponent8Row73 :
    Fin 6 → LaurentExponent (Fin 144) :=
![
  tropicalOverlapDegreeFiveExponent8 7 64 89 99 138,
  tropicalOverlapDegreeFiveExponent8 7 64 89 108 129,
  tropicalOverlapDegreeFiveExponent8 7 76 85 89 138,
  tropicalOverlapDegreeFiveExponent8 17 42 85 89 138,
  tropicalOverlapDegreeFiveExponent8 17 51 85 89 129,
  tropicalOverlapDegreeFiveExponent8 25 51 76 89 116
]

/-- Explicit shifted exponents of the six monomials from source `B_j`. -/
def tropicalOverlapSourceJExponent8Row73 :
    Fin 6 → LaurentExponent (Fin 144) :=
![
  tropicalOverlapDegreeFiveExponent8 7 65 85 102 138,
  tropicalOverlapDegreeFiveExponent8 7 65 85 111 129,
  tropicalOverlapDegreeFiveExponent8 7 76 85 89 138,
  tropicalOverlapDegreeFiveExponent8 17 42 85 89 138,
  tropicalOverlapDegreeFiveExponent8 17 51 85 89 129,
  tropicalOverlapDegreeFiveExponent8 26 51 76 85 119
]

set_option maxHeartbeats 10000000 in
/-- Kernel replay of the six shifted `B_i` exponents in row 73. -/
theorem tropicalOverlapSourceIExponent8_replay_row73 (j : Fin 6) :
    Pi.single tropicalOverlapProvenance8Row73.coordinateB (1 : ℤ) +
        tropicalMatchingLocalExponent8
          (tropicalBaseColoring8 tropicalOverlapProvenance8Row73.sourceI)
          (tropicalBaseMatching8 j) =
      tropicalOverlapSourceIExponent8Row73 j := by
  fin_cases j <;> decide

set_option maxHeartbeats 10000000 in
/-- Kernel replay of the six shifted `B_j` exponents in row 73. -/
theorem tropicalOverlapSourceJExponent8_replay_row73 (j : Fin 6) :
    Pi.single tropicalOverlapProvenance8Row73.coordinateA (1 : ℤ) +
        tropicalMatchingLocalExponent8
          (tropicalBaseColoring8 tropicalOverlapProvenance8Row73.sourceJ)
          (tropicalBaseMatching8 j) =
      tropicalOverlapSourceJExponent8Row73 j := by
  fin_cases j <;> decide

/-- The exact translated-source combination after exponent replay. -/
def tropicalOverlapSourceCombination8Row73 :
    LaurentPolynomial (Fin 144) :=
  tropicalOverlapProvenance8Row73.epsilon •
    ((∑ j : Fin 6,
        Finsupp.single (tropicalOverlapSourceIExponent8Row73 j) 1) -
      ∑ j : Fin 6,
        Finsupp.single (tropicalOverlapSourceJExponent8Row73 j) 1)

/-- Coefficientwise cancellation of the three common translated faces. -/
private theorem tropicalOverlapRelation8_sourceCombination_row73 :
    tropicalOverlapRelation8Row73 =
      tropicalOverlapSourceCombination8Row73 := by
  simp [tropicalOverlapRelation8Row73,
    tropicalOverlapSourceCombination8Row73,
    tropicalOverlapProvenance8Row73,
    tropicalOverlapSourceIExponent8Row73,
    tropicalOverlapSourceJExponent8Row73, Fin.sum_univ_succ]
  abel

/-- Staged kernel replay of `T_r = epsilon_r * (x_b B_i - x_a B_j)` for
row 73. -/
theorem tropicalOverlapRelation8_provenance_row73 :
    tropicalOverlapRelation8Row73 =
      tropicalOverlapProvenancePolynomial8
        tropicalOverlapProvenance8Row73 := by
  rw [tropicalOverlapRelation8_sourceCombination_row73]
  unfold tropicalOverlapSourceCombination8Row73
  unfold tropicalOverlapProvenancePolynomial8
  rw [tropicalOverlapTranslateBaseRelation8,
    tropicalOverlapTranslateBaseRelation8]
  simp_rw [tropicalOverlapSourceIExponent8_replay_row73,
    tropicalOverlapSourceJExponent8_replay_row73]

end

end MonochromaticQuantumGraphs.N8D3
