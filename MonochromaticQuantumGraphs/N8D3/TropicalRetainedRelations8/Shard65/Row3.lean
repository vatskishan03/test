import MonochromaticQuantumGraphs.N8D3.TropicalRetainedRelations8.Data

/-! Kernel replay for first-overlap row 328. -/

namespace MonochromaticQuantumGraphs.N8D3

noncomputable section

set_option maxRecDepth 100000

/-- Exact two-base-source provenance for overlap row 328. -/
def tropicalOverlapProvenance8Row328 : TropicalOverlapProvenance8 :=
  { epsilon := -1, sourceI := 172, coordinateB := 6, sourceJ := 173, coordinateA := 0 }

/-- The explicit sparse target polynomial in overlap row 328. -/
def tropicalOverlapRelation8Row328 : LaurentPolynomial (Fin 144) :=
  Finsupp.single
      (tropicalOverlapDegreeFiveExponent8 0 15 38 90 142) 1 +
    Finsupp.single
      (tropicalOverlapDegreeFiveExponent8 0 15 49 90 127) 1 +
    Finsupp.single
      (tropicalOverlapDegreeFiveExponent8 0 26 49 69 123) 1 +
    (-Finsupp.single
      (tropicalOverlapDegreeFiveExponent8 6 9 38 90 142) 1) +
    (-Finsupp.single
      (tropicalOverlapDegreeFiveExponent8 6 9 49 90 127) 1) +
    (-Finsupp.single
      (tropicalOverlapDegreeFiveExponent8 6 20 49 69 123) 1)

/-- Explicit shifted exponents of the six monomials from source `B_i`. -/
def tropicalOverlapSourceIExponent8Row328 :
    Fin 6 → LaurentExponent (Fin 144) :=
![
  tropicalOverlapDegreeFiveExponent8 0 6 59 104 142,
  tropicalOverlapDegreeFiveExponent8 0 6 59 115 127,
  tropicalOverlapDegreeFiveExponent8 0 6 69 90 142,
  tropicalOverlapDegreeFiveExponent8 6 9 38 90 142,
  tropicalOverlapDegreeFiveExponent8 6 9 49 90 127,
  tropicalOverlapDegreeFiveExponent8 6 20 49 69 123
]

/-- Explicit shifted exponents of the six monomials from source `B_j`. -/
def tropicalOverlapSourceJExponent8Row328 :
    Fin 6 → LaurentExponent (Fin 144) :=
![
  tropicalOverlapDegreeFiveExponent8 0 6 59 104 142,
  tropicalOverlapDegreeFiveExponent8 0 6 59 115 127,
  tropicalOverlapDegreeFiveExponent8 0 6 69 90 142,
  tropicalOverlapDegreeFiveExponent8 0 15 38 90 142,
  tropicalOverlapDegreeFiveExponent8 0 15 49 90 127,
  tropicalOverlapDegreeFiveExponent8 0 26 49 69 123
]

set_option maxHeartbeats 10000000 in
/-- Kernel replay of the six shifted `B_i` exponents in row 328. -/
theorem tropicalOverlapSourceIExponent8_replay_row328 (j : Fin 6) :
    Pi.single tropicalOverlapProvenance8Row328.coordinateB (1 : ℤ) +
        tropicalMatchingLocalExponent8
          (tropicalBaseColoring8 tropicalOverlapProvenance8Row328.sourceI)
          (tropicalBaseMatching8 j) =
      tropicalOverlapSourceIExponent8Row328 j := by
  fin_cases j <;> decide

set_option maxHeartbeats 10000000 in
/-- Kernel replay of the six shifted `B_j` exponents in row 328. -/
theorem tropicalOverlapSourceJExponent8_replay_row328 (j : Fin 6) :
    Pi.single tropicalOverlapProvenance8Row328.coordinateA (1 : ℤ) +
        tropicalMatchingLocalExponent8
          (tropicalBaseColoring8 tropicalOverlapProvenance8Row328.sourceJ)
          (tropicalBaseMatching8 j) =
      tropicalOverlapSourceJExponent8Row328 j := by
  fin_cases j <;> decide

/-- The exact translated-source combination after exponent replay. -/
def tropicalOverlapSourceCombination8Row328 :
    LaurentPolynomial (Fin 144) :=
  tropicalOverlapProvenance8Row328.epsilon •
    ((∑ j : Fin 6,
        Finsupp.single (tropicalOverlapSourceIExponent8Row328 j) 1) -
      ∑ j : Fin 6,
        Finsupp.single (tropicalOverlapSourceJExponent8Row328 j) 1)

/-- Coefficientwise cancellation of the three common translated faces. -/
private theorem tropicalOverlapRelation8_sourceCombination_row328 :
    tropicalOverlapRelation8Row328 =
      tropicalOverlapSourceCombination8Row328 := by
  simp [tropicalOverlapRelation8Row328,
    tropicalOverlapSourceCombination8Row328,
    tropicalOverlapProvenance8Row328,
    tropicalOverlapSourceIExponent8Row328,
    tropicalOverlapSourceJExponent8Row328, Fin.sum_univ_succ]
  abel

/-- Staged kernel replay of `T_r = epsilon_r * (x_b B_i - x_a B_j)` for
row 328. -/
theorem tropicalOverlapRelation8_provenance_row328 :
    tropicalOverlapRelation8Row328 =
      tropicalOverlapProvenancePolynomial8
        tropicalOverlapProvenance8Row328 := by
  rw [tropicalOverlapRelation8_sourceCombination_row328]
  unfold tropicalOverlapSourceCombination8Row328
  unfold tropicalOverlapProvenancePolynomial8
  rw [tropicalOverlapTranslateBaseRelation8,
    tropicalOverlapTranslateBaseRelation8]
  simp_rw [tropicalOverlapSourceIExponent8_replay_row328,
    tropicalOverlapSourceJExponent8_replay_row328]

end

end MonochromaticQuantumGraphs.N8D3
