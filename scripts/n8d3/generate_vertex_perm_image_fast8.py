...
* a balanced global dispatcher;
* a global correctness theorem assembled from the shard theorems; and
* fast, explicitly decidable relabeling and certificate predicates, together
  with a proof that a fast certificate establishes the existing trusted
  ``targetOrbitCertificateValid8`` proposition.
...
def emit_fast_predicate_module(lean_root: Path) -> Path:
    path = lean_root / "VertexPermImageFast8.lean"
    lines = [
...
def generate(output: Path) -> dict[str, object]:
    ...
    owned_outputs = (
        ...
        lean_root / "VertexPermImageFast8.lean",
    )
    ...
    fast_path = emit_fast_predicate_module(lean_root)
    paths = [*shard_paths, global_path, fast_path]
